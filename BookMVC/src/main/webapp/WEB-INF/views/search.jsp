...
<body class="bg-white">
	...
	<div class="container" style="max-width: 700px">
		...
		<form method="get"
			...
			<div class="input-group">
				<input type="text" class="form-control" name="q"
					...
					placeholder="Search by title, author or genre..." maxlength="200"
					required>
		...
		<c:if test="${results != null}">
			<p class="text-muted">
				<strong>${fn:length(results)}</strong> result(s) for "<strong><c:out
						value='${query}' /></strong>"
			</p>
			<c:choose>
						...
						<c:forEach items="${results}" var="b">
							<div class="list-group-item">
								<div class="d-flex justify-content-between align-items-center">
									<div>
										<strong><c:out value="${b.title}" /></strong> <span
											class="text-muted ms-2"> by <c:out value="${b.author}" />
										</span> <br> <small> <span class="genre-tag"> <c:out
													value="${b.genres}" />
										</span> <span class="text-muted ms-2"> <c:out
													value="${b.date}" />
										</span>
										</small>
									</div>
